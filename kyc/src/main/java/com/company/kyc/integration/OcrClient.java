...
public class OcrClient {
    ...
    //     result.setFraudScore(result.getRiskScore() > 10 ? 80 : 0); // simple heuristic for demo
    ...
    private OcrCombinedResponseDto parseCombinedResponse(String responseBody) throws Exception {
        ...
        // NEW: data lives at root level, not under "summary"
        JsonNode documentsNode = root.path("documents");
        JsonNode crossDocNode = root.path("cross_document_comparison");
        JsonNode decisionNode = root.path("decision");

        OcrCombinedResponseDto.OcrData ocrData = new OcrCombinedResponseDto.OcrData();

        /* ---------------- PAN ---------------- */
        JsonNode panNode = documentsNode.path("pan").path("normalized_data");
        if (!panNode.isMissingNode()) {
            OcrCombinedResponseDto.OcrData.PanData panData =
                    new OcrCombinedResponseDto.OcrData.PanData();

            panData.setName(panNode.path("name").asText(null));
            panData.setDate_of_birth(panNode.path("dob").asText(null));
            panData.setPan_no(panNode.path("pan").asText(null));
            panData.setFather_name(panNode.path("father_name").asText(null));

            ocrData.setPan(panData);
        }

        /* ---------------- AADHAAR ---------------- */
        JsonNode aadhaarNode = documentsNode.path("aadhaar").path("normalized_data");
        if (!aadhaarNode.isMissingNode()) {
            OcrCombinedResponseDto.OcrData.AadhaarData aadhaarData =
                    new OcrCombinedResponseDto.OcrData.AadhaarData();

            aadhaarData.setName(aadhaarNode.path("name").asText(null));
            aadhaarData.setDate_of_birth(aadhaarNode.path("dob").asText(null));
            aadhaarData.setAadhaar_no(aadhaarNode.path("aadhaar").asText(null));

            ocrData.setAadhaar(aadhaarData);
        }

        /* ---------------- BANK STATEMENT ---------------- */
        JsonNode bankNode = documentsNode.path("bank").path("normalized_data");
        if (!bankNode.isMissingNode()) {
            OcrCombinedResponseDto.OcrData.StatementData statementData =
                    new OcrCombinedResponseDto.OcrData.StatementData();

            statementData.setName(bankNode.path("customer_name").asText(null));
            statementData.setAccount_no(bankNode.path("account_number").asText(null));
            statementData.setIfsc_code(bankNode.path("ifsc").asText(null));
            statementData.setAddress(null); // not present in response
        ...
        // identity_score from cross_document_comparison.average_name_score
        result.setIdentityScore(crossDocNode.path("average_name_score").asInt());

        // risk_score and fraud_score from decision node
        double riskScore = decisionNode.path("risk_score").asDouble();
        result.setRiskScore(riskScore);
        result.setFraudScore(decisionNode.path("fraud_score").asInt());

        System.out.println("Risk Score: " + riskScore);

        // Status logic (same thresholds as before)
        if (riskScore > 5) {
            result.setStatus("Rejected");
        } else {
            result.setStatus("Approved");
        ...
        // message from decision.message
        result.setMessage(decisionNode.path("message").asText());

        // fraud signals from decision.signals
